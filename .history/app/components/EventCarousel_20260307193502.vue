...
// }>()
   ...
   const events = ref([
    ...
    // Adjust carousel width to account for extra cards
    const totalCards = carouselChildrens.length;

    // Add duplicate cards at the end for infinite scrolling
    carouselChildrens.slice(0, cardPerView).forEach(card => {
        carousel.insertAdjacentHTML("beforeend", card.outerHTML);
    });

    // Scroll to the starting position to show the first child
